Create a single horizontal sprite strip for the Codex app digital pet `tomori` in the state `review`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: a compact chibi schoolgirl digital pet inspired by Takamatsu Tomori: short muted lavender-gray bob hair with soft bangs, large gentle reddish eyes, small reserved mouth, grey Haneoka winter blazer with white lapel piping and gold buttons, white shirt, dark green tie with diagonal yellow stripes, dark green plaid pleated skirt with yellow grid lines, dark navy knee socks, brown loafers, shy calm posture; no rocks in the base pet or normal rows; only the review row may include one tiny gray pebble held close as a subtle Easter egg.
...
- This is the only animation row that may include the stone Easter egg: one tiny gray pebble, touching or held close to Tomori's hand, readable but secondary.
- Keep the pebble in the same frame slot as Tomori and attached by contact or overlap; never let it float separately, become a scene object, or appear in more than this review row.
