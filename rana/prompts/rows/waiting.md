Create a single horizontal sprite strip for the Codex app digital pet `rana-parfait` in the state `waiting`.
...
Animation action: patient waiting loop with small motion. In this waiting row only, include a tiny matcha parfait side prop in one short treat/eating beat: the parfait should be small, held close to or touching the pet's hands, and clearly secondary to the pet. Do not place the parfait as a detached separate object in a corner, do not make it larger than the pet's hands, and do not let it appear in any other row.
