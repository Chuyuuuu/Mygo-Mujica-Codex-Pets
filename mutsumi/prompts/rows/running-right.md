Create a single horizontal sprite strip for the Codex app digital pet `mutsumi` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: a tiny chibi pixel-art-adjacent companion inspired by Wakaba Mutsumi: long pale mint-white hair with straight bangs, muted golden eyes, calm slightly blank expression, a small silver hair clip above the left ear, wearing the blue-purple plaid dress from the two clothing reference images as the priority outfit, with cream sailor-style collar or bib, teal ribbon bow, compact skirt silhouette, tiny black socks and brown shoes; simplify all details for 192x208 readability; no text, no mask, no guitar, no bunny plush.
